...
public:
	...
	void drawSmallOrbit(void)
	{
		...
		glutSolidTorus(0.1, distance, 100.0, 100.0);
		glPopMatrix();
	}
...
void setup(void)
{
	...
	Image *sta = loadBMP("assets/stars.bmp");
	staTexture = loadTexture(sta);
	delete sta;
	Image *sun = loadBMP("assets/sun.bmp");
	sunTexture = loadTexture(sun);
	delete sun;
	Image *mer = loadBMP("assets/mercury.bmp");
	merTexture = loadTexture(mer);
	delete mer;
	Image *ven = loadBMP("assets/venus.bmp");
	venTexture = loadTexture(ven);
	delete ven;
	Image *ear = loadBMP("assets/earth.bmp");
	earTexture = loadTexture(ear);
	delete ear;
	Image *mar = loadBMP("assets/mars.bmp");
	marTexture = loadTexture(mar);
	delete mar;
	Image *jup = loadBMP("assets/jupiter.bmp");
	jupTexture = loadTexture(jup);
	delete jup;
	Image *sat = loadBMP("assets/saturn.bmp");
	satTexture = loadTexture(sat);
	delete sat;
	Image *ura = loadBMP("assets/uranus.bmp");
	uraTexture = loadTexture(ura);
	delete ura;
	Image *nep = loadBMP("assets/neptune.bmp");
	nepTexture = loadTexture(nep);
	delete nep;
	Image *plu = loadBMP("assets/pluto.bmp");
...
void orbitalTrails(void)
{
	...
	glRotatef(90.0, 1.0, 0.0, 0.0);
	glutSolidTorus(0.1, mer.distance, 100.0, 100.0);
	glutSolidTorus(0.1, ven.distance, 100.0, 100.0);
	glutSolidTorus(0.1, ear.distance, 100.0, 100.0);
	glutSolidTorus(0.1, mar.distance, 100.0, 100.0);
	glutSolidTorus(0.1, jup.distance, 100.0, 100.0);
	glutSolidTorus(0.1, sat.distance, 100.0, 100.0);
	glutSolidTorus(0.1, ura.distance, 100.0, 100.0);
	glutSolidTorus(0.1, nep.distance, 100.0, 100.0);
	glutSolidTorus(0.1, plu.distance, 100.0, 100.0);
	glPopMatrix();
}
